<h1 align="center">🧰 Chest</h1>

<p align="center">
  Open source terminal ecosystem<br>
  Emulator · TUI framework · Lisp scripting
</p>

---

## Apps

**🖥️ Portty** — [github.com/chestso/portty](https://github.com/chestso/portty)  
Modern terminal emulator — SDL3, GPU rendering, cross-platform

**💬 Mudlark** — [github.com/chestso/mudlark](https://github.com/chestso/mudlark)  
MUD/telnet client with embedded Lisp scripting

**🎹 Ditty** — [github.com/chestso/ditty](https://github.com/chestso/ditty)  
Embeddable Lisp interpreter and REPL with Flare syntax highlighting

---

## Libraries

**📟 Coffer** — [github.com/chestso/coffer](https://github.com/chestso/coffer)  
Virtual terminal engine — VT parser, grid, scrollback, reflow, sixel

**🫧 Boba** — [github.com/chestso/boba](https://github.com/chestso/boba)  
TUI library with Elm Architecture — the Bubbletea of C

---

🌐 [chest.so](https://chest.so)

---

_Free software, built in C and Lisp._
